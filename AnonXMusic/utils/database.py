...
autosholatdb = mongodb.autosholat
...
async def remove_autosholat(chat_id: int):
    await autosholatdb.delete_one({"chat_id": chat_id})


async def get_autosholat_chats():
    return await autosholatdb.find({}).to_list(length=1000)
